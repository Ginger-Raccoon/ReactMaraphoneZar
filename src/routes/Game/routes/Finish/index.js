                ...
                <div className={s.row}>
                    {
                        player1Cards.map((item) => (
                            item.card && <PokemonCard {...item.card} isActive className={s.card} />
                        ))
                    }
                </div>
                <button type="button" className={s.button} onClick={handleEndGame}>
                    End Game
                </button>
                <div className={s.row}>
                    {
                        player2Cards.map((item) => (
                            item.card && <PokemonCard
                                {...item.card}
                                isActive
                                className={cn(s.card, { [s.selected]: selectedCard && selectedCard.card.id === item.card.id })}
                                selectPokemon={() => {
